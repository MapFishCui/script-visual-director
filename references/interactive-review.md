...
这是本地浏览器编辑器；使用 Skill 的 Python 虚拟环境和标准库 HTTP 服务，无新增依赖，无模型密钥。页面保存中文修改、显示对应系统文本和版本差异、记录逐镜及阶段确认。页面不自动唤起 助手，不调用视频或翻译服务。

同一对话中，用户保存后回复“同步修改”，继续本项目，无需再次调用 skill。普通“确认”只确认当前明确展示的对象和版本；若当前有多个待确认对象且指向不明，先明确对象。用户点击页面确认按钮也构成实际确认依据。助手 做 UI 测试时只能在测试副本点击确认，不能代替真实用户确认生产项目。
...
新项目在登记分析后，以空镜表 review-init --workflow layout-first-v1（CLI 默认）初始化，再 review-serve 展示分析与阶段进度。先用 review-storyboard 首次导入中文导演稿；用户依次确认中文导演稿、平面图和整场调度白模后，编译并确认最终系统分镜；不重新初始化页面，不创建占位镜头。详见 [新流程协议](layout-first-workflow.md)。已有审核文件保留旧流程；不能重新初始化覆盖。可用 --analysis-files 明确列出剧情确认文件，避免把后续生产资料加入分析指纹。
...
1. 新流程先在页面确认中文导演稿，再确认平面图和整场调度白模；之后编译并确认最终系统分镜。旧项目沿原记录继续。
2. 按 [三项分镜规范](three-part-storyboard.md) 编辑场景、镜头运动、人物运动；内部创作依据与时长可展开查看或编辑，点“保存中文修改”。
3. 保存会生成新中文版本，保留旧系统文本作对照，撤销该镜头批准；修改时长还会将后续镜头的系统文本标记为待同步，避免保留过期切点。新流程中文修改会使中文稿与依赖其版本的布局、调度确认过期，并列出候选影响；仅系统翻译更新不改变上游中文稿确认。旧项目沿用原失效规则。
4. 回当前对话说“同步修改”。助手 同步完成后刷新页面，检查中文和目标文本的对应关系，完成导演检查后，再点“确认当前镜头”。
...
#助手读取 request，撰写真实翻译/目标格式以及影响判断后：
...
- 候选影响不等于必须重做。助手 判断后在 invalidate_assets/layouts 明确列出失效对象；无需修改时可以留空，但须写原因。
...
分镜阶段使用 [导演组与任务](generation-groups.md) 建立并确认组方案。网页先显示导演组合并时长与完整段落意图，再显示内部视频段任务、段内切点、系统文本和中文对照，可编辑组名、镜号与导演意图后保存，再回当前对话说“同步分组”。保存会撤销组确认；拆组/合组由助手一次提交全段方案。分组有独立版本与历史；未保存草稿保留在浏览器会话中，过期草稿可复制合并。原分镜变化使旧组方案失效，正式资产流程与导演包导出保持待办。
...
助手 每个确认节点以及最终交付均打开或刷新审阅 URL，检查当前标题、成果与版本，并告诉用户要确认哪一项。资产确认暂时通过对话指定资产及版本，助手 记录真实依据后刷新图库。页面不会调用模型或自动推进后台生产。不要把“给了路径”“服务已启动”说成“用户已经看到”。
