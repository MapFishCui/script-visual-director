...
本文保留布局优先项目的命令与历史记录。当前默认九宫格流程见 [项目入口](../README.md)。
...
台词逐句描述语气，例如慵懒、愤怒、害羞，同时交代音量、速度、停顿和情绪变化。描述声音不等于制作音频；本 skill 可生成完整关键画面九宫格作为视觉引导，不生成配音或最终 AI 视频。
...
新项目先确认中文导演稿，再确认布局，使用 blocking-* 生成整场调度白模；最终系统分镜确认后才使用 previs-* 生成逐镜摄影机预演。两阶段均可明确跳过，但不记为已验证。命令与数据见 [完整阶段协议](layout-first-workflow.md)。旧项目保持原流程，不自动迁移。
...
人物四图规格保持不变；场景图按 [预演到场景资产](scene-reference.md) 绑定已确认机位与几何结构，先确认美术基准再扩展所需方位。
...
完整协议与 JSON 字段见 [生成分组](generation-groups.md)。

## 11. 详细规范入口

- [技能入口](../SKILL.md)：助手 的执行顺序与完成标准。
- [项目设计](../PROJECT_DESIGN.md)：整体设计与数据流。
- [操作与数据约定](asset-schema.md)：资产登记、依赖、修订和确认。
- [交互审核](interactive-review.md)：中文同步、版本和迁移。
- [导演检查](director-review.md)：节奏、心理、表演与逐句语气。
- [本地预演](local-previs.md)：动画适配协议、绑定和检查。
- [人物四图](character-reference.md)：人物资产规格。
- [生成分组](generation-groups.md)：跨工具分组、组预演与导演包导出。
- [H3 交接](h3-handoff.md)：目标格式与交付边界。
...
完整命令和JSON方案见 [跨集资产继承](series-continuity.md)。


可复用白模组件已提供 `model-validate`、`model-build`、`model-bind`：结构、骨骼 IK、接触与持物绑定、代理穿插检查。能力范围与操作见 [建模组件](modeling-components.md)。


现成模型下载与跨项目留存：见 [三维资产库](model-library.md)。提供 library-list/fetch/pin/inspect/check/restore/bundle，官方 Snow 可按需下载；模型二进制存本地缓存，不进入 Git。
