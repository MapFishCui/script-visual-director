...
人物三维资产必须从原作者或发布机构官网取得成品模型与成熟绑定，并通过资产库下载、留存、锁定版本及记录来源许可。禁止自行建人体，也禁止用内置 actor、胶囊、球体或圆柱组件拼成人物，包括粗略调度预演。找不到适合体型的官方模型、下载失败或动作适配缺失时，报告该部分 blocked；不得回退组件人物或用静止站姿冒充动作完成。官网已下载且可核实的缓存可复用，无需每次重新下载。此要求针对人物，墙体、门和机关仍可使用结构组件。

下文旧 actor 接口与演示仅保留用于历史算法回归，不是可选制作方案；生产构建与绑定拒绝 type=actor。使用 type=asset、category=actor 导入官网模型。
...
需要成熟人物或现成物件时，优先使用 [三维资产库](model-library.md) 与 type=asset。基础 actor 不得用于制作或调度代理。导入模型的碰撞与统一 IK 适配未完成时报告 partial，不能把没有代理的模型当作检查通过。
