...
新完整流程先确认页面里的中文导演稿，再确认平面图、整场调度白模，之后编译并确认最终系统分镜；简单场景也可用简洁布局，轻量情节分镜不要求制作平面图。场景资产依赖稳定且检查过的布局版本。

分镜之前的观察机位和人物路线可用 shot=null；没有必要时 cameras、paths 可为空，不造占位镜头。
...
这个命令记录助手的视觉检查，不是额外要求用户批准。自动碰撞报告不会自动写成“人工检查通过”。检测到实体相交可能是有意遮挡，应结合原文解释，而不是机械移走物体。
