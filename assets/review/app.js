...
const statuses = {draft:'中文方案 · 白模后编译',pending_sync:'待同步',review:'待确认',needs_direction:'待导演检查',approved:'已确认'};
...
function updateDirty(){if(!state?.shots.length)return;dirty=JSON.stringify(values())!==JSON.stringify(Object.fromEntries(fields.map(f=>[f,shot().zh[f]??(f==='duration'?null:'')])));
$('saveState').textContent=dirty?'有未保存修改 · 切换镜头前请保存':'已保存到本地项目';
$('save').disabled=!dirty||busy; $('approve').disabled=dirty||busy||shot().status!=='review'||!state.stage_status.analysis||!shot().quality?.ready||!state.storyboard_prerequisites;
...
function stagebar(){
 ...
 const modern=state.workflow==='layout-first-v1';
 const entries=modern?[['analysis','① 中文导演稿'],['layout','② 平面图'],['blocking','③ 整场调度白模'],['storyboard','④ 最终分镜 / 系统原文'],['previs','⑤ 逐镜摄影机预演'],['assets','⑥ 资产生产']]:[['analysis','① 剧情分析'],['storyboard','② 分镜'],['layout','③ 平面布局'],['previs','④ 逐镜预演'],['assets','⑤ 资产生产']];
 for(const [key,name] of entries){
  const done=key==='storyboard'?(state.shots.length>0&&state.shots.every(s=>s.status==='approved')&&state.storyboard_prerequisites):key==='assets'?state.production?.complete:state.stage_status[key];
  const cfg=state[key];const skipped=['blocking','previs'].includes(key)&&done&&cfg.required===false;
  const item=el('div',undefined,'stage'+(done?' done':''));
  item.append(el('b',name),el('span',skipped?'已跳过（未验证）':done?'✓':'待完成'));
  if(['blocking','previs'].includes(key)){
   const ready=state.stage_status.layout&&(key==='blocking'||(state.shots.length>0&&state.shots.every(s=>s.status==='approved')&&state.storyboard_prerequisites));
   item.append(el('small',key==='blocking'?'确认整场行动与空间，不锁定剪辑':'按已确认分镜生成摄影机参考候选'));
   const run=state[key+'_run'];if(run&&!done)item.append(el('small',run.status==='running'?'后台运行中':run.status==='failed'?'运行失败，请查看日志':'已有运行结果，待检查或确认'));
   if(!done||skipped){const button=el('button',skipped?'改为生成':'生成白模视频','secondary');button.disabled=busy||!ready;button.onclick=()=>guard(()=>choosePrevis('generate',key));item.append(button);}
   if(!skipped){const button=el('button',key==='blocking'?'跳过调度白模，继续分镜':'跳过逐镜预演，继续资产','secondary');button.disabled=busy||!ready;button.onclick=()=>guard(()=>choosePrevis('skip',key));item.append(button);}
   if(!done&&cfg.required&&cfg.files.length){const button=el('button','确认已查看的视频','secondary');button.disabled=busy||!ready;button.onclick=()=>guard(()=>confirmStage(key));item.append(button);}
  }else if(['analysis','layout'].includes(key)&&!done){const button=el('button','确认','secondary');button.disabled=busy||!state.initialized||(key==='layout'&&(!state.stage_status.analysis||(!modern&&!state.shots.every(s=>s.status==='approved'))));button.onclick=()=>guard(()=>confirmStage(key));item.append(button);}
  box.append(item);
 }
}
async function choosePrevis(choice,phase='previs'){try{busy=true;state=await api('/api/'+phase+'-choice',{choice,revision:state.revision});render();notice(choice==='skip'?'已记录跳过，本阶段未验证。':'已选择生成。回当前对话说“继续”；渲染在后台执行。');}catch(e){notice(e.message,true);}finally{busy=false;stagebar();updateDirty();}}
...
$('targetText').textContent=s.target.text||(state.workflow==='layout-first-v1'&&!state.storyboard_prerequisites?'先确认左侧中文导演稿、平面图和白模；之后编译系统分镜及中文对照。':'尚未编译目标系统文本。请回当前对话说“同步修改”。');$('targetTranslation').textContent=s.target.translation_zh||'尚未补齐系统原文的中文对照。请回当前对话说“同步修改”。';$('targetHelp').textContent=s.status==='pending_sync'&&s.target.text?'以下系统原文及中文对照均为旧版，尚未反映本次中文修改，不能确认。':'系统原文和下方中文对照绑定同一版本；左侧中文导演稿可编辑。';$('targetNote').textContent=s.target.note||'';$('sources').replaceChildren();for(const url of state.system.sources){const a=el('a','目标系统参考指南 ↗');a.href=url;a.target='_blank';a.rel='noopener noreferrer';$('sources').append(a,document.createElement('br'));}
const beforeReference=state.workflow==='layout-first-v1'&&!state.storyboard_prerequisites;
$('draftHelp').textContent=beforeReference?'正文只写场景、镜头运动、人物运动。先保存并在顶部确认中文导演稿，再查看平面图与白模；最终系统分镜在白模后编译。':'正文只写场景、镜头运动、人物运动。修改后保存，再回当前对话说“同步修改”。';
$('targetQuality').hidden=beforeReference;
if(beforeReference){$('targetState').textContent='白模后编译';$('targetHelp').textContent='当前先确认左侧中文方案，尚未到最终系统分镜阶段。';if(!s.target.translation_zh)$('targetTranslation').textContent='系统分镜编译后在这里显示中文对照。';}
$('approve').textContent=state.workflow==='layout-first-v1'?'确认最终系统分镜':'确认当前镜头';
const impact=$('impact');impact.replaceChildren();if(s.impact){for(const [key,name] of [['shots','相邻镜头'],['layouts','关联布局'],['assets','候选资产']])impact.append(el('p',`${name}：${s.impact[key]?.join('、')||'无'}`));impact.append(el('p',s.impact.review_note||s.impact.note||'等待助手检查','muted'));}else impact.append(el('p','同步时检查动作、位置、台词时长及关联资产。','muted'));
...
async function saveEdit(){if(!$('editor').reportValidity())return false;try{busy=true;$('save').disabled=true;const key=draftKey();const result=await api('/api/edit',{id:sid,zh:values(),revision:state.revision});sessionStorage.removeItem(key);state=result;dirty=false;render();notice('中文修改已保存。请回当前对话说“同步修改”；同步后在此查看差异并确认。');return true;}catch(e){notice(e.message+' 当前输入已保留。',true);return false;}finally{busy=false;updateDirty();stagebar();}}
...
async function navigateSection(id,label){
 ...
 try{
  ...
  if(!target||target.closest('[hidden]'))message=current?'尚未生成'+label+'。请在当前对话中继续制作。':'项目已有更新；请先保存或复制编辑，再刷新项目查看'+label+'。';
  else{
...
function renderQuality(){
 ...
 box.append(el('p','保存后回当前对话说“同步修改并检查节奏和表演”。字段完整不代表艺术质量；导演检查依据可在此查看。','muted'));
}
...
function renderGroups(){
 ...
 if(!data){box.append(el('p','尚未建立分组方案。可回当前对话说“整理生成分组”。','muted'));return;}
 box.append(el('p',`分组 v${data.revision} · ${data.adapter} · ${data.approved?'方案已确认':'待同步或确认'}`));
 box.append(el('p','导演组对应一次合并输出，可为53秒、48秒或更长；内部视频段分别生成。H3的15秒预算只检查内部任务。修改后回当前对话说“同步分组”。','muted'));
 ...
 for(const group of data.schedule){
  ...
  const save=el('button','保存本任务修改','secondary');save.type='button';save.onclick=()=>guard(async()=>{
   try{busy=true;const plan=data.groups.map(g=>({...g,references:g.references.map(r=>({...r})),shots:[...g.shots]}));const selected=plan.find(g=>g.id===group.id);
    ...
    state=await api('/api/groups-plan',{revision:data.revision,project_revision:state.revision,adapter:data.adapter,shared_references:data.shared_references,groups:plan,...(data.director_groups?.length?{director_groups:data.director_groups}:{})});sessionStorage.removeItem(storageKey);render();notice('分组修改已保存，请回当前对话说“同步分组”。');
...
function renderOverview(){
 ...
 }else box.append(el('p','剧集共享资产：尚未绑定。制作后续集时，可在助手指定共享库并选择要继承的版本。','muted'));
 const phases=el('div',undefined,'actions');
 for(const [label,value] of [
  [state.workflow==='layout-first-v1'?'中文导演稿':'剧情分析',state.stage_status.analysis?'已确认':'待确认'],
  ...
  ...(state.workflow==='layout-first-v1'?[['整场调度白模',state.stage_status.blocking?(state.blocking.required?'已确认':'已跳过'):'待选择／完成／确认']]:[]),
  ['逐镜预演',state.stage_status.previs?(state.previs.required?'已确认':'已跳过'):'待选择／完成／确认'],
  ['基准与扩展资产',p?.total?`${p.completed}/${p.total} 已完成`:'尚未登记'],
  ['交付',p?.complete?'待核对导出说明':'待前置完成']])phases.append(el('span',`${label}：${value}`,'tag'));
 box.append(phases);
 box.append(el('p','每次确认后回当前对话说“继续”。页面不会自动调用模型。资产与交付资料覆盖整个项目，不随当前镜头筛选。','muted'));
 ...
 if(!p?.total)gallery.append(el('p','尚未登记资产需求。前置阶段确认完毕后，由助手整理清单并生成基准图。','muted'));
 ...
 for(const a of p?.assets||[]){const card=el('div',undefined,'resource');card.append(el('strong',`${a.name} · ${a.id} · v${a.version}`));
  ...
  cards.append(card);
 }gallery.append(cards,el('p','确认基准时，请回当前对话 指明资产名称或编号及版本；确认会登记到项目，再解锁依赖它的扩展图。','muted'));box.append(gallery);
...
function renderDirectorGroups(box,data){
 ...
 if(!data.director_schedule?.length)box.append(el('p','当前为旧版短任务列表，尚未划分导演组。回当前对话说“按剧情组织导演组”；保留原任务和镜头。','qualityError'));
 for(const group of data.director_schedule||[]){
  ...
  const save=el('button','保存导演组修改','secondary');save.type='button';save.onclick=()=>guard(async()=>{
   try{busy=true;const directors=data.director_groups.map(g=>({...g,tasks:[...g.tasks]}));const selected=directors.find(g=>g.id===group.id);
    ...
    state=await api('/api/groups-plan',{revision:data.revision,project_revision:state.revision,adapter:data.adapter,shared_references:data.shared_references,groups:data.groups,director_groups:directors});sessionStorage.removeItem(key);render();notice('导演组已保存，回当前对话 同步内部任务文本和衔接。');
